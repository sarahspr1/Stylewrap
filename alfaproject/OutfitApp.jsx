...
import { Home, Shirt, Calendar, CalendarDays, Heart, User, ChevronLeft, ChevronRight, Camera, Plus, Trash2, Pencil, Search, TrendingUp, Palette, Layers, X, Bell, Shield, Phone, LogOut, Check, DollarSign, Tag, Wind, Gem, Waves } from "lucide-react";
import { PieChart, Pie, Cell, ResponsiveContainer } from "recharts";

function CatIcon({ cat, size=12, color="#ABABAB" }){
          ...
          if(entry?.logged){
              ...
              return (<>
                ...
                {editEntry.items.map((item,i)=>(
                  <div key={i} style={{ background:C.surface,borderRadius:0,padding:12,marginBottom:10,border:`1px solid ${C.border}` }}>
                    ...
                    {(()=>{ const outfitColors=[...new Set((editEntry.items||[]).map(it=>it.color).filter(c=>c&&colorHex[c]))]; const pickerColors=outfitColors.length>0?outfitColors:COLORS; return <div style={{ display:"flex",gap:5,flexWrap:"wrap",marginBottom:8 }}>{pickerColors.map(col=><button key={col} onClick={()=>updateItem(i,"color",col)} title={col} style={{ width:52,border:item.color===col?`2px solid ${C.sage}`:col==="White"?`1px solid ${C.border}`:`1px solid transparent`,cursor:"pointer",padding:0,background:"transparent",flexShrink:0,overflow:"hidden" }}><div style={{ width:"100%",height:32,background:colorHex[col] }}/><div style={{ padding:"3px 4px",background:C.white,textAlign:"left" }}><div style={{ fontSize:9,fontWeight:700,color:C.ink,lineHeight:1.2,whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{col}</div><div style={{ fontSize:8,color:C.sub,fontFamily:"monospace",lineHeight:1.3 }}>{colorHex[col]}</div></div></button>)}</div>; })()}
            ...
            return (<>
              <div style={{ display:"flex",gap:10,marginBottom:10 }}>
                {entry.photo&&<div style={{ width:"38%",flexShrink:0,borderRadius:0,overflow:"hidden",aspectRatio:"3/4" }}><img src={entry.photo} alt="Outfit" style={{ width:"100%",height:"100%",objectFit:"cover",display:"block" }}/></div>}
                <div style={{ flex:1,display:"flex",flexDirection:"column",gap:6,minWidth:0 }}>
                  {(entry.style||entry.formalityLevel||entry.season)&&<div style={{ display:"flex",flexWrap:"wrap",gap:4 }}>{entry.style&&<span style={{ fontSize:10,fontWeight:700,color:C.sage,background:C.sage+"18",padding:"3px 8px",borderRadius:0,border:`1px solid ${C.sage}30` }}>{entry.style}</span>}{entry.formalityLevel&&<span style={{ fontSize:10,fontWeight:700,color:"#fff",background:"#7A6A9A",padding:"3px 8px",borderRadius:0 }}>{entry.formalityLevel}</span>}{entry.season&&<span style={{ fontSize:10,fontWeight:700,color:"#fff",background:"#5A85C4",padding:"3px 8px",borderRadius:0 }}>{entry.season}</span>}</div>}
                  {entry.notes&&<div style={{ background:C.surface,padding:"6px 8px",border:`1px solid ${C.border}` }}><p style={{ fontSize:10,fontWeight:700,color:C.sub,textTransform:"uppercase",letterSpacing:"0.08em",margin:"0 0 2px" }}>Notes</p><p style={{ fontSize:11,color:C.ink,margin:0,lineHeight:1.4 }}>{entry.notes}</p></div>}
                  <p style={{ fontSize:10,fontWeight:700,color:C.sub,textTransform:"uppercase",letterSpacing:"0.1em",margin:0 }}>What I wore</p>
                  {selectedItemIdxs.size>0&&<button onClick={removeSelected} style={{ fontSize:11,fontWeight:700,color:C.red,background:"#FEF0EF",border:"none",borderRadius:0,padding:"3px 8px",cursor:"pointer",fontFamily:"inherit",alignSelf:"flex-start" }}>Remove {selectedItemIdxs.size}</button>}
                  {entry.analysing?<span style={{fontSize:12,color:C.sub}}>Analysing…</span>:cats.length>0?<div style={{ display:"flex",flexDirection:"column",gap:6 }}>{cats.map(cat=>(<div key={cat}><div style={{ display:"flex",alignItems:"center",gap:4,marginBottom:3 }}><CatIcon cat={cat} size={11} color={C.sub}/><span style={{ fontSize:10,fontWeight:700,color:C.sub,textTransform:"uppercase",letterSpacing:"0.08em" }}>{cat}</span></div>{grouped[cat].map((item,i)=>{ const hex=item.color&&colorHex[item.color]?colorHex[item.color]:null; const isSel=selectedItemIdxs.has(item._idx); const isFav=favourites.some(f=>(f.name||"").trim().toLowerCase()===(item.name||"").trim().toLowerCase()); const wearCount=knownItems[(item.name||"").trim().toLowerCase()]?.count;
return <div key={i} style={{ width:"100%",background:isSel?C.sage+"14":C.surface,borderRadius:0,padding:"5px 8px",marginBottom:3,border:isSel?`1.5px solid ${C.sage}`:`1px solid ${C.border}`,display:"flex",alignItems:"center",gap:6 }}><div onClick={()=>toggleItem(item._idx)} style={{ width:14,height:14,borderRadius:"50%",border:isSel?"none":`1.5px solid ${C.border}`,background:isSel?C.sage:"transparent",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,cursor:"pointer" }}>{isSel&&<span style={{ color:"#fff",fontSize:9,lineHeight:1 }}>✓</span>}</div>{hex&&<div style={{ width:10,height:10,background:hex,flexShrink:0,border:item.color==="White"?`1px solid ${C.border}`:"none" }}/>}<span onClick={()=>toggleItem(item._idx)} style={{ fontSize:11,color:C.ink,fontWeight:500,flex:1,cursor:"pointer",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap" }}>{item.name||String(item)}</span>{wearCount>1&&<span style={{ fontSize:9,fontWeight:700,color:C.sage,background:C.sage+"18",borderRadius:0,padding:"1px 5px",flexShrink:0 }}>{wearCount}x</span>}<button onClick={e=>{ e.stopPropagation(); onToggleFavourite&&onToggleFavourite(item); }} style={{ background:"none",border:"none",cursor:"pointer",padding:2,flexShrink:0,display:"flex",alignItems:"center" }}><Heart size={13} color={isFav?C.red:"#ccc"} fill={isFav?C.red:"none"}/></button></div>; })}</div>))}</div>:<span style={{ fontSize:11,color:C.sub }}>No items — tap Edit to add</span>}
                </div>
              </div>
              <div style={{ display:"flex",gap:8,marginBottom:0 }}>
                <button onClick={()=>{ setEditEntry({ style:entry.style||null, formalityLevel:entry.formalityLevel||null, season:entry.season||null, notes:entry.notes||"", items:(entry.items||[]).map(item=>typeof item==="object"&&item?{...item}:{ name:String(item||""),category:"Other",color:null }) }); setEditMode(true); setSelectedItemIdxs(new Set()); }} style={{ flex:1,height:42,borderRadius:0,border:`1.5px solid ${C.border}`,background:C.white,color:C.ink,fontSize:13,fontWeight:700,cursor:"pointer",fontFamily:"inherit",display:"flex",alignItems:"center",justifyContent:"center",gap:6 }}><Pencil size={14} color={C.sage}/>Edit Outfit</button>
                <DangerBtn onClick={()=>{ setPhotoData(p=>{ const n={...p}; delete n[toKey(selectedDate)]; return n; }); setShowModal(false); }}>Remove</DangerBtn>
              </div>
            </>);
          }
          return (<>
